...
TestResults = namedtuple("TestResults", ["name", "hostnames", "medians"])
...
def load_nc_file(hostnames: Iterable[Any], path: Path) -> Dict[int, pd.DataFrame]:
...
def load_test_results(directory: Path) -> TestResults:
    ...
    medians = load_nc_file(hostnames, directory.joinpath("network_median.nc"))
    return TestResults(name=directory.basename(), hostnames=hostnames, medians=medians)
...
def main():
    ...
    test_results = load_test_results(Path("/home/shibbiry/Dropbox/documents/msu/bachelors_thesis_cluster_topology/test_results/2017-02-12__118_nodes/"))
